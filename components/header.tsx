...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-[#111143]">
      <nav className="max-w-7xl mx-auto  py-6 flex items-center justify-between relative">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm hover:text-[#040C8C] transition-colors uppercase text-[#dad1c8]"
            >
              {item.label}
            </Link>
          ))}
        ...
        {/* Linha inferior dentro do container */}
        <div className="absolute bottom-0 left-0 right-0 h-px bg-[#dad1c8]" />
      </nav>
